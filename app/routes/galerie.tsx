	...
	return (
		<div className="container-responsive py-12 md:py-20">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
				{gallery.map((image) => (
					<button
						key={image.id}
						type="button"
						className="group relative aspect-video cursor-pointer overflow-hidden rounded-xl shadow-sm hover:shadow-lg transition-all w-full p-0 border-0 text-left"
			...
			{selectedImage && (
				<button
					type="button"
					className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4 transition-opacity duration-300 animate-in fade-in w-full h-full border-0 cursor-default"
				...
				>
					<button
						type="button"
						className="absolute top-4 right-4 text-white hover:text-gray-300 transition-colors bg-white/10 rounded-full p-2 cursor-pointer"
						onClick={(e) => {
							e.stopPropagation();
							setSelectedImage(null);
						}}
					...
					<div
						className="max-w-5xl max-h-[90vh] relative cursor-auto"

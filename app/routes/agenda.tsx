...
export default function Agenda() {
	...
	// biome-ignore lint/suspicious/noExplicitAny: simple component
	const EventCard = ({ event }: { event: any }) => (
		<Card
			className={`p-0 overflow-hidden flex flex-col md:flex-row transition-all hover:shadow-md border-l-4 ${event.type === "association" ? "border-l-primary" : "border-l-accent"}`}
		>
			<div className="bg-gray-50 p-6 md:w-48 flex flex-col items-center justify-center text-center border-b md:border-b-0 md:border-r border-gray-ui/50 shrink-0">
				<span
					className={`text-3xl font-bold mb-1 ${event.type === "association" ? "text-primary" : "text-accent"}`}
				>
